...
> **Nota:** esta carpeta (`lab/`) es una de las dos herramientas del repositorio `netsecure-lab` — la otra es `firewall-sim/`. Ambas se publican juntas desde la raíz del repositorio; ver el [README general](../README.md) para la vista completa. Este laboratorio queda accesible en `https://tu-usuario.github.io/netsecure-lab/lab/` (no en la raíz del sitio — la raíz es la página de selección).

1. En GitHub, entra a **Settings → Pages** del repositorio.
2. En **Build and deployment → Source**, selecciona **Deploy from a branch**.
3. En **Branch**, elige `main` (o la que uses) y la carpeta `/ (root)`.
4. Guarda. GitHub mostrará la URL pública (algo como `https://tu-usuario.github.io/netsecure-lab/`) en unos minutos — esa es la página de inicio con los 2 botones; el laboratorio en sí queda en `.../lab/`.
5. Comparte con los estudiantes el enlace directo al laboratorio (`.../lab/`) o el de la página de inicio, según prefieras. No requiere configuración adicional: GitHub Pages sirve los archivos por HTTPS, lo que permite que `fetch()` cargue el JSON sin problemas.
...
4. El caso puede seleccionarse agregando `?caso=banca` a la URL de la aplicación (por ejemplo, `https://tu-usuario.github.io/netsecure-lab/lab/?caso=banca`). Si no se indica ningún parámetro, se carga el primer caso de la lista.
